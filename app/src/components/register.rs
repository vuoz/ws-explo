...
#[component]
pub fn register_view() -> impl IntoView {
    ...
    let on_sub = move |_| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            let res = match calls::register::send_register(name, pass).await {
                Ok(res) => res,
                Err(_) => return,
            };
            ...
            let key = res.key;
            storage.set_item("cli_key", &key).unwrap();
    ...
    view! {
        <div class="w-full h-screen flex items-center content-center justify-center">
            <div class="w-[50%] outline outline-gray-900 h-[50%]  p-[10%]  space-y-4">
                <p class="text-white font-bold text-left">Register</p>
                <input
                    type="text"
